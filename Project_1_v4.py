...
""" 
  ...
  need time to recharge. Enemies will shoot at you. Avoid them and their projectiles!
    ...
    Version: 4
    - Added boss class and level!
        - Player cannot proceed until boss is killed
        - Player has damage value, must sustain attack on boss to do significant damage
        - Boss has health value
        - buttons added to compensate new level
    - Added splash screen holder, splash screen TBD
    - Code is much more efficient overall
    - Changed backgrounds to JPG as the levels run much more smoothly in that format
...
ENEMY_START_X_BUFFER = 800 #the closest X distance from the left side of the environment that the enemies can spawn
...
SPLASH_SCREEN_WAIT = 300 #in milliseconds
...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self, difficulty):
        ...
        elif(self.difficulty == "hard" or self.difficulty == "boss"):
...
class Boss(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        self.updateHealth() #updat the health of the boss object
    ...
    def moveBoss(self):
        ...
        print(self.dy, self.facingY, self.rect.centery)
    ...
    def game(self, difficulty, score): # the game difficulty and score are passed to method game
        ...
        while keepGoing:
            ...
            if(bossLevel): #setup boss level collisions
                bossHitPlayer = pygame.sprite.collide_rect(boss, player)
                lightningHitBoss = pygame.sprite.collide_rect(lightning, boss)
            ...
            if(bossLevel):  #check for boss level collisions
                if(bossHitPlayer and not player.invulnerable):
                    player.invulnerable = True # make the player take no damage after being hit
                    player.displayHealthElapsed = 0                 # for a short period of time
                    player.displayHealth = True
                    player.health -= boss.damage #take away health
                    sndPlayerHurt.play()
                if(lightningHitBoss and player.attacking):
            ...
            if(endGame):
                ...
                elif(difficulty == "hard" or difficulty == "boss"):
...
class Menu():
    ...
    def mainMenu(self):
        ...
        while playing == "menu":
            ...
            self.mediumButton.create_button(SCREEN,  DARK_BLUE,     100, 300, 200,    75,     0,      "Medium"   , WHITE)
            self.hardButton.create_button(  SCREEN,  DARK_RED,      100, 400, 200,    75,     0,      "Hard"     , WHITE)
            self.bossButton.create_button(  SCREEN,  WHITE,         100, 500, 200,    75,     0,      "Boss"     , BLACK)
...
def main():
    ...
    while playing != "quit":
        ...
        elif(playing == "easy" or playing == "medium" or playing == "hard" or playing == "boss"):
            difficulty = playing
